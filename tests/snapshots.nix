...
simpleTest {
  ...
  testScript =
    ...
    ''
      ...
      client.succeed(
          "${env} disnix-nixops-client --target server --activate --type wrapper ${wrapper}"
      )

      # Take a snapshot
      client.succeed(
          "${env} disnix-nixops-client --target server --snapshot --type wrapper ${wrapper}"
      )
      ...
      result = client.succeed(
          "${env} disnix-nixops-client --target server --query-all-snapshots --container wrapper --component ${wrapper} | wc -l"
      )
      ...
      result = client.succeed(
          "${env} disnix-nixops-client --target server --query-latest-snapshot --container wrapper --component ${wrapper}"
      )
      lastSnapshot = result[:-1]
      lastResolvedSnapshot = client.succeed(
          "${env} disnix-nixops-client --target server --resolve-snapshots {}".format(
              lastSnapshot
          )
      )
      ...
      result = client.succeed(
          "${env} disnix-nixops-client --target server --print-missing-snapshots --container wrapper --component ${wrapper} {} wrapper/wrapper/foo | wc -l".format(
              lastSnapshot
          )
      )
      ...
      result = client.succeed(
          "${env} disnix-nixops-client --target server --export-snapshots {}".format(
      ...
      client.succeed(
          "${env} disnix-nixops-client --target server --clean-snapshots --container wrapper --component ${wrapper}"
      )
      result = client.succeed(
          "${env} disnix-nixops-client --target server --query-all-snapshots --container wrapper --component ${wrapper} | wc -l"
      )
      ...
      client.succeed(
          "${env} disnix-nixops-client --target server --import-snapshots --localfile --container wrapper --component wrapper {}".format(
              tmpDir[:-1]
          )
      )
      ...
      # uploaded state (2)
      client.succeed(
          "${env} disnix-nixops-client --target server --restore --type wrapper ${wrapper}"
      )
      ...
      client.succeed(
          "${env} disnix-nixops-client --target server --deactivate --type wrapper ${wrapper}"
      )
      # Delete the state and check if it is not present anymore.
      client.succeed(
          "${env} disnix-nixops-client --target server --delete-state --type wrapper ${wrapper}"
      )
      ...
      client.succeed(
          "${env} disnix-nixops-client --target server --clean-snapshots --container wrapper --component ${wrapper} --keep 0"
      )
      result = client.succeed(
          "${env} disnix-nixops-client --target server --query-all-snapshots --container wrapper --component ${wrapper} | wc -l"
      )
